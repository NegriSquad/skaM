...
async function getPublicKey() {
    // В реальном проекте нужно получать с сервера
    // Для демонстрации используем VAPID ключи
    // Вы можете сгенерировать свои на https://web-push-codelab.glitch.me/
...
function showBrowserNotification(title, body, chatId) {
    ...
    // Не показываем уведомление, если чат активен
...
function toggleNotificationSettings() {
    ...
    if (permission === 'default') {
        ...
        alert(state.notificationsEnabled ? '✅ Уведомления включены' : '❌ Уведомления отключены');
...
// ============================================
// МОДИФИЦИРОВАННАЯ ФУНКЦИЯ ДЛЯ ОТПРАВКИ УВЕДОМЛЕНИЙ
// ============================================

// Добавляем в функцию sendMessage уведомление для получателя
async function sendOrUpdateMessage() {
    ...
    try {
        ...
        if (state.activePartner && state.activePartner.uid) {
            ...
            // Сохраняем уведомление в базе для получателя
...
// ============================================
// ОТПРАВКА ПУШ-УВЕДОМЛЕНИЙ ЧЕРЕЗ FIREBASE
// ============================================

// Функция для отправки уведомления через Firebase Cloud Messaging
async function sendPushNotification(chatId, partnerId, message, senderName) {
    try {
        // Получаем подписку получателя
        const subscriptionsSnap = await db.ref(`push_subscriptions/${partnerId}`).once('value');
        const subscriptions = subscriptionsSnap.val() || {};
        
        // Если есть подписки - отправляем через FCM
        // Для реального использования нужно настроить Firebase Cloud Messaging
        // или использовать Web Push Protocol напрямую
        
        // Отправляем браузерное уведомление (если получатель - текущий пользователь)
        const currentUserId = state.user?.uid;
        if (partnerId === currentUserId) {
            showBrowserNotification(senderName || 'Новое сообщение', message, chatId);
            playNotificationSound();
        }
        
        console.log('Уведомление отправлено для:', partnerId);
...
// ============================================
// СЛУШАТЕЛЬ НОВЫХ СООБЩЕНИЙ ДЛЯ УВЕДОМЛЕНИЙ
// ============================================

// Добавляем слушатель для новых сообщений в фоновом режиме
...
function updateUnreadCount() {
    ...
    db.ref(`notifications/${state.user.uid}`).once('value', (snapshot) => {
        ...
        // Обновляем счетчик в заголовке страницы
        if (unread > 0) {
        ...
        // Показываем на иконке вкладки если есть
...
// Добавляем в handleAuthState инициализацию слушателя уведомлений
...
// Обновляем resetSession для очистки слушателей уведомлений
function resetSession() {
    detachListeners();
    
    // Отключаем слушатель уведомлений
    if (state.user) {
...
// Добавляем отметку о прочтении при открытии чата
...
// ... (функции loginUser, registerUser, renderDialogs и т.д.)
...
function openChat(chatId, partner) {
    ...
    // Прокручиваем вниз после открытия чата
    setTimeout(() => {
        scrollMessagesToBottom(false);
    }, 200);
}
...
function updateScrollState() {
    ...
    // Если нет активного чата - скрываем кнопку
    if (!state.activeChatId) {
    ...
    // Проверяем, есть ли переполнение (контент больше видимой области)
    const hasOverflow = scrollHeight > clientHeight + 20;
    
    // Проверяем, находится ли пользователь внизу (с допустимым отклонением)
    const isAtBottom = scrollHeight - scrollTop - clientHeight < 50;
    state.isAtBottom = isAtBottom;
    
    // Показываем кнопку ТОЛЬКО если:
    // 1. Есть переполнение
    // 2. Пользователь НЕ внизу
    // 3. Кнопка не была скрыта после нажатия
...
window.addEventListener('resize', () => {
    // Обновляем состояние прокрутки при изменении размера
